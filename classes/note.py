...
class Note:
    ...
    def __init__(self, abs_pitch=None, abs_duration=None,
                 tone=None, octave=None, notelength=None, scale=None):
        ...
        if abs_pitch < self.scale.MIN_PITCH:
            raise PitchError("Pitch {0} is less than min " \
                "{1}.".format(abs_pitch, self.scale.MIN_PITCH))
        if abs_pitch > self.scale.MAX_PITCH:
            raise PitchError("Pitch {0} is greater than max " \
                "{1}.".format(abs_pitch, self.scale.MAX_PITCH))
